...
describe("briefsmith app", () => {
  ...
  it("returns a signed structured fallback brief when ANTHROPIC_API_KEY is missing", async () => {
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.result).toContain("## Executive Summary");
    expect(body.result).toContain("## Key Metrics");
    expect(body.result).toContain("## Conclusion");
    expect(body.result).toContain("## Confidence Score");
    expect(body.result).toContain("## Sources");
    expect(body.result).toContain("_Generated by Twiin Agent Network_");
  });
  ...
  it("returns a signed anthropic brief when API key is set", async () => {
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.result).toContain("Executive Brief");
  });
});
