...
const EnvSchema = z.object({
  ...
  AGENT_NAME: z.string().min(1).max(32).optional().default("discord-bot@twiin"),
  AGENT_COST_STT: z.string().optional().default("0.15"),
  REGISTRATION_DEPOSIT_STT: z.string().optional().default("5"),
});

export type DiscordBotEnv = z.infer<typeof EnvSchema> & {
  AGENT_COST_WEI: bigint;
  REGISTRATION_DEPOSIT_WEI: bigint;
};

export function loadEnv(source: NodeJS.ProcessEnv = process.env): DiscordBotEnv {
